...
export async function generateScraper(directory: string, name: string, ts: boolean) {
...
export default async function(ayakashi: IAyakashiInstance, input: ScraperInput, params: ScraperParams) {
    await ayakashi.goTo(input.page);
    ayakashi
        .select("name")
        .where({itemprop: {eq: "name"}});
