...
export async function generateProject(projectDir: string, useCurrentFolder: boolean, ts: boolean) {
    ...
    if (ts) await mkdirp("src");
    opLog.info("generating config file");
    if (ts) {
        const config = pathResolve(projectDir, "src", "ayakashi.config.ts");
        await writeFile(config, getConfigTS());
    } else {
        await writeFile("ayakashi.config.js", getConfig());
    }
    opLog.info("generating package.json");
    await writeFile("package.json", ts ? getpackageJsonTS() : getpackageJson());
    if (ts) {
        opLog.info("generating tsconfig.json");
        await writeFile("tsconfig.json", getTsConfig());
    }
    if (ts) {
        const tsSource = pathResolve(projectDir, "src");
        await generateScraper(tsSource, "githubAbout", ts);
        await generateScript(tsSource, "getPage", ts);
    } else {
        await generateScraper(projectDir, "githubAbout", ts);
        await generateScript(projectDir, "getPage", ts);
    }
    await writeFile(".gitignore", ts ? getGitIgnoreTS() : getGitIgnore());
    let npm = "npm";
...
function getConfigTS() {
    return (
`import {Config} from "@ayakashi/types";
